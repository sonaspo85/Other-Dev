...
$(document).ready(function(){
	...
	var result_body = message[language].result_body;
	...
	if($.cookie('switch_search') !== null) {
		if($.cookie('switch_search') == "sw1") {
			$("#id_results2").css("display","none"); $("#id_results").css("display","block"); $("#no_results").css("display","block"); 
			$("#sw1").addClass("switch");
		} else {
			$("#id_results2").css("display","block"); $("#id_results").css("display","none"); $("#no_results").css("display","none"); 
			$("#sw2").addClass("switch");
		};
	...
	function search_keyword_toc() {	
		...
		$("#id_json_results").html($("#id_json_results").html().replace(regex, "<span class='highlight'> "+user_keyword_lowcase+"</span>")); //옮기기 전 검색 단어에 대한 색상 표기				
		...
		t_html = "";
		t_html = t_html + "<div id='search_box'><div id='search_kind1'><div id='chapter1'><div id='title'></div></div></div><div id='title_line'></div><div id='search_kind1'><div id='titlebody'></div><div id='view2'></div></div></div>";
		...
		$("#id_json_results #root").find("div:containsNoCase('" + user_keyword_lowcase + "')").each(function(){
			...
			switch($(this).attr("id")) {
				case "topic_h1": insert_result_kind("topic_h1", $(this)); break;					
				case "topic_h2": insert_result_kind("topic_h2", $(this)); break;					
		...
		$("#id_results2 #titlebody:eq(0)").text(result_body);
	...
	$('#recent_keywords li').live('click', function() {
		...
		$("#id_results").html("");$("#id_results2").html(""); //초기화
		init();
		search_keyword_toc();
	});
	...
	//Setting UI text
	$("#sw2").text(switch_gen); $("#sw1").text(switch_toc);
