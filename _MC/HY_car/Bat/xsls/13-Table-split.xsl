...
<xsl:stylesheet 
	...
	<xsl:template match="Table">
        ...
        <xsl:variable name="des_cell" select="descendant::p[matches(@class, '^Description_Cell_8.5pt_C$')]" />
		<xsl:variable name="C_img_text_LR" select="descendant::span[matches(@class, 'C_img_text_(left)?(right)?')]" />
				...
				<xsl:choose>
					...
					<xsl:when test="$count_img = count($quad1_pos) and 
						...
						<xsl:for-each select="node()[*[matches(@class, '^Img-Center$')]][node()]">
							...
							<xsl:element name="Table">
                                ...
                                <xsl:if test="$des_cell[position() = $current_pos]">
                                    <xsl:apply-templates select="$des_cell[position() = $current_pos]/parent::*" />
                                </xsl:if>
					...
					<xsl:when test="matches(@class, 'Table_Product_Quad') and 
						...
						<xsl:choose>
							...
							<xsl:when test="descendant::*[matches(@class, 'C_img_text_(left)?(right)?')]">
								<xsl:for-each select="node()[matches(@class, 'Cell_ImgCenter')][node()]">
									<xsl:variable name="current_pos" select="position()" />
									...
									<xsl:element name="Table">
										...
										<xsl:if test="$C_img_text_LR">
											<Cell>
												<xsl:apply-templates select="$C_img_text_LR/parent::*" />
											</Cell>
										</xsl:if>
					...
					<xsl:when test="matches(@class, 'Table_Product_Quad') and 
							  		Cell[not(node())]">
						<xsl:choose>
							<xsl:when test="ancestor::Cell and 
									  		Cell[not(node())]/preceding-sibling::node()[1]/*[matches(@class, '_noText')]">
								<xsl:copy>
									<xsl:apply-templates select="@*, node()" />
								</xsl:copy>
                            </xsl:when>
					...
					<xsl:when test="Cell[1][*[matches(@class, '^Quad1$')]]/following-sibling::Cell[1][*[matches(@class, '^Img-Center$')]]">
						<xsl:for-each select="node()[*[matches(@class, '^Quad1$')]][node()]">
							...
							<xsl:element name="Table">
								...
								<xsl:if test="position() = last()">
                                    <xsl:if test="count(ancestor::Table/Cell[*[matches(@class, 'Description_Cell_8.5pt_C')]]) = 1">
                                        <xsl:apply-templates select="$des_cell/parent::*" />
                                    </xsl:if>
									<xsl:apply-templates select="$cls_degn_pos/parent::*" />
								</xsl:if>
							</xsl:element>
