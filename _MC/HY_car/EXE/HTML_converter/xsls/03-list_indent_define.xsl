			...
			<xsl:for-each select="$str0/node()">
				<xsl:copy>
					...
					<xsl:for-each select="node()">
						<xsl:choose>
							<xsl:when test="self::*[name()!='Table'][not(matches(@class, 'Heading'))][following-sibling::node()[1][matches(@class, 'group_indent')]]">
								<xsl:copy>
									<xsl:apply-templates select="@*, node()" />
									<xsl:copy-of select="following-sibling::node()[1][matches(@class, 'group_indent')]/node()" />
								</xsl:copy>
							</xsl:when>
							<xsl:when test="self::*[matches(@class, 'group_indent')] and preceding-sibling::node()[1][name()!='Table'][not(matches(@class, 'Heading'))]">
							</xsl:when>
