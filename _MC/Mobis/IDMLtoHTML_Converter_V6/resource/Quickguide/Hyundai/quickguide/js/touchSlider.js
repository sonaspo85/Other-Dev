	...
	var touchSlider = {
		...
		init: function() {
			...
			if(this.opts.paging) {
				...
				for(var i=1; i<=len; ++i) {
					paging += '<button type="button" class="ts-paging-btn"></button>';
				}
		...
		touchmove: function(e) {
			...
			if((e.type == 'touchmove' && e.originalEvent.touches.length <= 1) || e.type == 'mousemove') {
				...
				var last = sectionLength -2
				var footer = sectionLength -1

				if(w < h) {
					// 최초실행 범위
					if(this._top >= -80 && this._top <= 0) {
						$(".section").css({
							"transition":"all 0.1s", 
							"transform":"translateY(" + this._top + "px)",
							"-moz- transition":"all 0.1s", 
							"-moz- transform":"translateY(" + this._top + "px)"
						});
					}
					...
					if($(".section").eq(0).hasClass("active")) {
					...
					} else if($(".section").eq(1).hasClass("active")) {
						if(this._top >= -150 && this._top <= 80) {
							$(".section").css({
								"transition":"all 0.1s", 
								"transform":"translateY(" + this._top + "px)",
								"-moz- transition":"all 0.1s",
					...
					} else if($(".section").eq(last).hasClass("active")) {
						if(this._top >= -50 && this._top <= 150) {
							$(".section").css({
								"transition":"all 0.2s", 
								"transform":"translateY(" + this._top + "px)",
					...
					} else if($(".section").eq(footer).hasClass("active")) {
						if(this._top >= 0 && this._top <= 50) {
							$(".section").css({
								"transition":"all 0.2s", 
								"transform":"translateY(" + this._top + "px)",
								"-moz- transition":"all 0.2s",
						...
						if(this._top >= -150 && this._top <= 150) {
							$(".section").css({
								"transition":"all 0.1s", 
								"transform":"translateY(" + this._top + "px)",
								"-moz- transition":"all 0.1s",
		...
		touchend: function(e) {
			...
			if(this._scroll) {
				...
				if(w < h) {
					if(this._top > 0){
						$.scrollify.previous();
						$(".section").css({"transition":"all 0.2s", "transform":"translateY(0px)"});
					} else {
						$.scrollify.next();
						$(".section").css({"transition":"all 0.2s", "transform":"translateY(0px)"});
					}
