...
<xsl:stylesheet 
    ...
    <xsl:template match="topic[p|ul]">
        ...
        <xsl:variable name="str3">
            <xsl:for-each-group select="$str2/*" group-adjacent="boolean(self::ul[matches(@class, 'UL1_1(-BlankNone)?$')])">
                <xsl:choose>
                    <xsl:when test="current-group()[1][matches(@class, 'UL1_1(-BlankNone)?$')]/following-sibling::*[1][matches(@class, 'UL1_1(-BlankNone)?$')]">
                        <ul>
                            <xsl:apply-templates select="current-group()/@class"/>
                            <xsl:apply-templates select="current-group()/node()"/>
                        </ul>
        ...
        <topic>
            ...
            <xsl:for-each-group select="$str3/*" group-adjacent="boolean(self::ul[matches(@class, '^UL1_1$')][not(@video-after) and not(@video-before)])">
                <xsl:choose>
