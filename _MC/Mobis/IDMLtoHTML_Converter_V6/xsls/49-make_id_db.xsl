...
<xsl:stylesheet 
    ...
    <xsl:variable name="h12s" select="/topic//div[matches(@class, 'swipe_inner_wrap')]/div[matches(@class, 'heading1')]/descendant-or-self::*[matches(name(), 'h(1|2)')]" />
    ...
    <xsl:template match="/">
        ...
        <xsl:result-document href="{$id_dbFile}">
            ...
            <xsl:for-each select="$h12s">
                <xsl:variable name="ancesTopic" select="ancestor::topic[@file][1]" />
                <xsl:variable name="file">
                    <xsl:value-of select="$ancesTopic/@file" />
                </xsl:variable>
                
                <xsl:variable name="chapter_id">
                    <xsl:variable name="fileLastID" select="tokenize(replace($ancesTopic/@file, '.html', ''), '_')[last()]" />
                        ...
                        <xsl:when test="name() = 'h2'">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    ...
                                    <xsl:value-of select="count(ancestor::div[parent::topic[@file]]/preceding-sibling::div)"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                    </xsl:choose>
                ...
                <xsl:variable name="title">
                    <xsl:value-of select="replace(replace(ancestor::topic[@idml]/topic[1]/h0, '&#xa;', ''),'^\s+', '')" />
                </xsl:variable>
                ...
                <xsl:text disable-output-escaping='yes'> "chapter_id": "</xsl:text>
                <xsl:value-of select="$chapter_id"/>
                <xsl:text disable-output-escaping='yes'>",&#xA;</xsl:text>
                
                <xsl:text disable-output-escaping='yes'> "h2_no": "</xsl:text>
                <xsl:value-of select="$h2_no"/>
                <xsl:text disable-output-escaping='yes'>",&#xA;</xsl:text>
