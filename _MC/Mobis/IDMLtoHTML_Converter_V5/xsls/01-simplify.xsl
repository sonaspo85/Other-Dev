...
<xsl:stylesheet 
	...
	<xsl:variable name="idml_import" select="document(concat(ast:getName(base-uri(.), '/'), '/../resource/Customize/', 'idml_import.xml'))/root" />
			...
			<xsl:element name="docs">
               ...
               <xsl:apply-templates select="//HyperlinkURLDestination" />
				...
				<xsl:for-each select="docs/doc1">
					<xsl:if test="not(ends-with(@doc-name, 'Cover') or 
								  ends-with(@doc-name, 'TOC') or 
								  ends-with(@doc-name, 'Index'))">
						<xsl:element name="doc">
							<xsl:attribute name="idml" select="concat(@doc-name, '.idml')"/>
							<!--<xsl:call-template name="call.stroy">
								<xsl:with-param name="stroyList" select="Document/@StoryList"/>
								<xsl:with-param name="docData" select="."/>
							</xsl:call-template>-->
	...
	<xsl:template match="docs" mode="chapter_sort">
		...
		<xsl:copy>
			...
			<xsl:for-each select="$idml_import/chapter">
				<xsl:variable name="same">
					<xsl:choose>
						<xsl:when test="current()[@type = $spec1]">
							<xsl:for-each select="topic[parent::*/@type = $spec1]">
				...
				<xsl:variable name="differ">
					<xsl:choose>
						<xsl:when test="not(parent::*/chapter[@type = $spec1]) and 
										 current()[@type != $spec1]">
    ...
    <!--<xsl:template name="call.stroy">
        <xsl:param name="stroyList"/>
        <xsl:param name="docData"/>
        <xsl:for-each select="tokenize($stroyList,' ')">
            <xsl:variable name="storyID" select="."/>
            <xsl:apply-templates select="$docData/idPkg:Story/Story[@Self eq $storyID][.!=' ']"/>
        </xsl:for-each>
    </xsl:template>-->
    ...
    <xsl:template match="Story">
        <xsl:choose>
            ...
            <xsl:when test="count(descendant::Content) eq 1 and empty(descendant::Content[1]/text())"/>
            <xsl:otherwise>
                <xsl:copy copy-namespaces="no">
    ...
    <xsl:template match="CharacterStyleRange">
        <xsl:choose>
           <xsl:when test="count(descendant::Content) eq 1 and empty(descendant::Content[1]/text())"/>
			...
			<xsl:when test="preceding-sibling::*[1][name()='XMLElement'] and following-sibling::*[1][name()='XMLElement'] 
					  		and count(node()) = 1 and child::Br">
			</xsl:when>
