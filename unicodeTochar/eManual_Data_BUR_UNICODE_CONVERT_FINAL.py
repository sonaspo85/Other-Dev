# Python code to search .CategoryData.xml files in current 
# folder (We can change file type/name and path 
# according to the requirements.

#Sabe this file where extraction data is placed, the list should be generated after
import os
import io 
import re
import argparse
...
for root, dirs, files in os.walk(dir_path): 
    for file in files:  
        ...
        # change the extension from 'CategoryData.xml' to  
        # the one of your choice. 
		# .xml files
        if file.endswith('.xml') or file.endswith('.js') or file.endswith('.html'):
            openFolderName = root+'\\'+str(file)
            print ('\n Folder Path: ' +openFolderName )
            
            baseFolderName = re.findall(r"((?<=Result\\).*$)",root)
            fileName=""
            folderName = str(openFolderName)
            folderName = folderName.replace('\'','')
            folderName = folderName.replace('[','')
            folderName = folderName.replace(']','')            
            folderName = openFolderName.split('\\')
            print(folderName[len(folderName) -2])
            
            mydoc = open(openFolderName,encoding="utf8")
            mylines = mydoc.readlines() 
            
			
            #finalPath = dir_path+"\\"+"BUR_NEW\\"+folderName[len(folderName) -2] +"\\"

            if not os.path.exists(openFolderName):
                os.makedirs(openFolderName)
            with io.open(openFolderName, 'w+', encoding='utf8') as writer:
                for line in mylines:
                    writer.write(zg2uni(line))
