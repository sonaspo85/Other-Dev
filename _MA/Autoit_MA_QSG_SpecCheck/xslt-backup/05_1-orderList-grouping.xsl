...
<xsl:stylesheet
    ...
    exclude-result-prefixes="xs son functx xsi"
    version="2.0">
	...
	<xsl:template match="*[*[matches(@class, '^UnorderList')]]">
		<xsl:copy>
			<xsl:apply-templates select="@*" />
			<xsl:for-each-group select="node()" group-adjacent="boolean(self::*[matches(@class, '^UnorderList')])">
				<xsl:choose>
					<xsl:when test="current-group()[1][matches(@class, '^UnorderList')][following-sibling::*[1][matches(@class, '^UnorderList')]]">
						<ol>
