...
<xsl:stylesheet
	...
	<xsl:variable name="directory" select="collection(concat(son:getpath(base-uri(.), '/'), $midName, '/?select=designmap.xml;recurse=yes'))" />
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="$foreach_direc/pos">
			...
			<xsl:result-document href="{concat($cur_dir, $midName, '/', $foreach_direc/pos[$i], '/', 'designmap1.xml')}" method="xml">
				<xsl:variable name="Dmap" select="document(concat($cur_dir, $midName, '/', $foreach_direc/pos[$i], '/', 'designmap.xml'))/Document" />
