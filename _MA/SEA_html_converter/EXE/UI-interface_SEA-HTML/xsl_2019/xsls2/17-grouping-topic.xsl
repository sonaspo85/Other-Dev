<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="chapter">
        <chapter>
            <xsl:sequence select="ast:group(*, 2)" />
        </chapter>
    </xsl:template>
    ...
    <xsl:function name="ast:normalized">
        ...
        <xsl:value-of select="replace(normalize-unicode(replace(replace(lower-case(normalize-space(replace($value, '\+', '_plus'))), '[!@#$%&amp;();:.,’?]+', ''), '[ &#xA0;]', '_'), 'NFKD'), '\P{IsBasicLatin}', '')"/>
    </xsl:function>
