...
var gName, target, cCell, sWidth, sp;
for (var i = 0; i < doc.allGraphics.length; i++) {
    // allGraphics: 문서에 포함된 모든 그래픽을 나열
...
var imgCount = doc.allGraphics.length - 2; //마스터 이미지 2개 제외
for (var i=0;i<imgCount;i++) {
    gr = doc.allGraphics[i];
    gName = gr.itemLink.name;
    target = gr.parent;

    if (gName.indexOf('smart-ui-') > -1) {
        target.absoluteHorizontalScale = 35;
        target.absoluteVerticalScale = 35;
    } else {
...
var varfile = app.activeScript.path + "/xml/" + langcode + ".xml";
// if (!varfile.exists) {
