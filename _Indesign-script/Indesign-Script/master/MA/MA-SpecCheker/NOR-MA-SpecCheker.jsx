...
var w = new Window ('palette {text: "NOR 사양 Check"}');
	//w.main = w.add ('group');
	...
	group02.orientation = "row";
		btn_02 = group02.add("button", [0, 0, 150, 30], "법인 주소 확인");
...
btn_01.onClick = function specChecker01() {
	if(app.documents.length < 1){
		...
		for (var i=0; i < doc.length; i++){
			...
			app.findTextPreferences.findWhat = "Ta ut batteriet";
...
btn_02.onClick = function () {
	...
	for (var i=0; i < doc.length; i++){
		if (doc[i].name == dest_file) {
			...
			try {
				app.findTextPreferences = app.changeTextPreferences = NothingEnum.NOTHING;
				app.findTextPreferences.findWhat = "Trenger du hjelp eller har spørsmål, henviser vi til";
			...
			//alert ("찾았다!");
			app.findTextPreferences = app.changeTextPreferences = NothingEnum.NOTHING;
			app.findTextPreferences.findWhat = "Opphavsrett";
	...
	function main() {
		...
		var source_file = File("C:/MA-WikiSpec/NOR/MA_online_UM_Nor_CorporateAddress_spec.indd"); // 사양문장 템플릿 문서 열기
		app.open(source_file);
		var source_doc = app.documents.item("MA_online_UM_Nor_CorporateAddress_spec.indd");
