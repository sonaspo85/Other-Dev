...
var w = new Window ('palette {text: "DAN 사양 Check"}');
	//w.main = w.add ('group');
...
btn_01.onClick = function specChecker01() {
	if(app.documents.length < 1){
		...
		for (var i=0; i < doc.length; i++){
			...
			app.findTextPreferences.findWhat = "Fjerne batteriet";
...
btn_04.onClick = function () {
	...
	for (var i=0; i < doc.length; i++){
		if (doc[i].name == dest_file) {
			...
			try {
				app.findTextPreferences = app.changeTextPreferences = NothingEnum.NOTHING;
				app.findTextPreferences.findWhat = "Behøver du hjælp, eller har du spørgsmål, er du";
			...
			//alert ("찾았다!");
			app.findTextPreferences = app.changeTextPreferences = NothingEnum.NOTHING;
			app.findTextPreferences.findWhat = "Ophavsret";
	...
	function main() {
		...
		var source_file = File("C:/MA-WikiSpec/DAN/MA_online_UM_Dan_CorporateAddress_spec.indd"); // 사양문장 템플릿 문서 열기
		app.open(source_file);
		var source_doc = app.documents.item("MA_online_UM_Dan_CorporateAddress_spec.indd");
