...
var w = new Window ("palette","한국어 조사 검사 ver.1.1");
	w.current = w.add ('statictext {text: "검사 개수: ", characters: 30, justify: "center"}');
	w.button = w.add("button", [0, 0, 180, 25], "시작");
var g1 = w.add("group");
var btn01 = g1.add("button", [0, 0, 85, 25], "를 → 을");
var btn03 = g1.add("button", [0, 0, 85, 25], "을 → 를");

var state = 0;
w.current.text = "검사 개수: " + state;

btn01.onClick = function() {
	app.selection[0].characters[-2].contents = "을";
}

btn03.onClick = function() {
	app.selection[0].characters[-2].contents = "를";
}
...
w.button.onClick = function() {
	...
	for (var i=state; i<myFound.length; i++) {
		state++;
		w.current.text = "검사 개수: " + state + " of " + myFound.length;
