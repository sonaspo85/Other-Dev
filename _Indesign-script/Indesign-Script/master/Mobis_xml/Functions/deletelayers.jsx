...
function DeleteLayers(docuType) {
	var docCount;
	if (docuType == "activeDocument") {
		docCount = 1;
		DeleteLayersMain(docCount);
...
function DeleteLayersMain(docCount) {
	...
	var myDiag = new Window("dialog", "레이어 삭제");
			...
			if (docCount == 1) { // 현재 활성화된 문서
				for (var k=0; k<sCount; k++) {
					if (app.documents[0].layers.item(visibleitems[k]).isValid) {
						app.documents[0].layers.item(visibleitems[k]).remove();
				...
				for (n=0; n<docCount; n++) {
					myProg.text = app.documents[n].name + ' 레이어 삭제 중...';
					myProg.pbar.value = n + 1;
					for (k=0; k<sCount; k++) {
						if (app.documents[n].layers.item(visibleitems[k]).isValid) {
							app.documents[n].layers.item(visibleitems[k]).remove();
