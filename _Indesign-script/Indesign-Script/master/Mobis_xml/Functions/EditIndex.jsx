...
function SetTopics(myDoc, topics) {
    var tempId = 1;
    var errorCount = 0;
    var parentName, ancientName;
    for (var j = 0 ; j <topics.length ; j++) {
        try {
            var topic = topics[j];
            var topicName = topic.name;
            var pageReference = topic.pageReferences[0];
            if (pageReference != null) {
                var sourceText = pageReference.sourceText;
                ...
                newitem.xmlAttributes.add('pageReferenceType', Number(pageReference.pageReferenceType) + '');
            ...
            for (var k=1; k<pageReference.length; k++) {
    ...
    try {
        ...
        for (var i = 0 ; i < pageItems.length ; i++) {
            try {
                ...
                errorMsgs.push('SetIndexFromXML 함수 오류1: ' + myDoc.name + '문서 - ' + ex + ":" + ex.line);
                errorCount++;
            }
        }
        ...
        for (var i = 0 ; i < pageItems.length ; i++) {
            try {
                ...
                errorMsgs.push('SetIndexFromXML 함수 오류2: ' + myDoc.name + '문서 - ' + ex + ":" + ex.line);
                errorCount++;
            }
        }
        ...
        for (var i = 0 ; i < pageItems.length ; i++) {
            try {
                ...
                errorMsgs.push('SetIndexFromXML 함수 오류3: ' + myDoc.name + '문서 - ' + ex + ":" + ex.line);
                errorCount++;
            }
        }
        if (errorCount > 0) {
