...
function HideLayers(docuType) {
	var docCount;
	if (docuType == "activeDocument") {
		docCount = 1;
		HideLayersMain(docCount);
...
function HideLayersMain(docCount) {
	...
	var myDiag = new Window("dialog", "레이어 숨기기");
			...
			if (docCount == 1) { // 현재 활성화된 문서
				for (var k=0; k<sCount; k++) {
					if (app.documents[0].layers.item(visibleitems[k]).isValid) {
						app.documents[0].layers.item(visibleitems[k]).visible = false;
				...
				for (n=0; n<docCount; n++) {
					myProg.text = app.documents[n].name + ' 레이어 숨기기...';
					myProg.pbar.value = n + 1;
					for (k=0; k<sCount; k++) {
						if (app.documents[n].layers.item(visibleitems[k]).isValid) {
							app.documents[n].layers.item(visibleitems[k]).visible = false;
