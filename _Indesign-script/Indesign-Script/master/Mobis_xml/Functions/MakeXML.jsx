...
function ImportXML(myDoc) {
    ...
    try {
        ...
        myDoc.xmlImportPreferences.importStyle = XMLImportStyles.mergeImport;
...
// added by Sunhill for correcting style overflow bug in InDesign
// date: 2023-02-17
function remedyStyleOverflow(myDoc) {
   var found = myDoc.xmlElements[0].evaluateXPathExpression('descendant::*[starts-with(name(), "C_")][not(Image)]');
   for (var i = 0; i < found.length; i++)
   {
       app.select(found[i]);
       var s1 = app.selection[0].insertionPoints[-1];
       app.select(found[i].parent);
       var s2 = app.selection[0].insertionPoints[-1];
       var range = s1.parentStory.insertionPoints.itemByRange(s1, s2);
       range.applyCharacterStyle(myDoc.characterStyles[0], true);
       range.clearOverrides();
   }
}
