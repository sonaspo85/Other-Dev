...
for (var n=0; n<bookcontents.length; n++) {
	if (bookcontents[n].name.indexOf("Readmefirst") != -1 || bookcontents[n].name.indexOf("TOC") != -1 || bookcontents[n].name.indexOf("Trademarks") != -1 || bookcontents[n].name.indexOf("Opensource") != -1) {
		continue
	}
...
function exportXMLfromheading() {
	...
	for (var i=0; i<story.length; i++) {
		...
		for (var j=0; j<paras.length; j++) {
			...
			try {
				if (paraStyle.indexOf("Heading1-Basic") != -1) {
				...
				} else if (paraStyle.indexOf("Heading2-Basic") != -1) {
					item = <item file={docfName} depth="2" style={paraStyle} contents={pContents}></item>;
					BMs.appendChild(item);
				} else if (paraStyle.indexOf("Heading1-Sublink") != -1) {
					item = <item file={docfName} depth="1" style={paraStyle} contents={pContents}></item>;
					BMs.appendChild(item);
				}
			} catch (e) {
