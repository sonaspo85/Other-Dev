...
//~ 			}
			...
			function exportByIdentifier (markerOrPrefix) {
				...
				for (var i = 0; i < sections.length; i++) {
					...
					marker = markerOrPrefix ? sections[i][markerOrPrefix]+'_' : '';
					f = File (base + '_' + marker + pages[first].name + '-' + pages[last].name + '.pdf');
					app.pdfExportPreferences.pageRange = '+'+(first+1) + '-' + '+'+(last+1);
					...
					switch (data.export_type) {
						...
						case 'pages': {
							...
							for (j = 0; j < doc.pages.length; j++) {
								app.pdfExportPreferences.pageRange = '+' + (j+1);
		...
		function select_items (book) {
			...
			var w = new Window ('dialog', book.name + ': PDF 출력기', undefined, {closeButton: false});
				...
				var p1 = g1.add('panel {text: "문서 출력", alignChildren: "left"}');
					var whole_docs = p1.add('radiobutton', undefined, '문서별 출력하기');
					var page_by_page = p1.add ('radiobutton', undefined, '페이지별로 출력하기');
					var sections = p1.add('radiobutton', undefined, '개별 섹션 별로 출력하기');
					var dummygroup = p1.add ('group {orientation: "column"}');
						dummygroup.margins.left = 25;
						var markers = dummygroup.add ('radiobutton', undefined, '파일 이름의 섹션 마커 사용');
						var prefixes = dummygroup.add ('radiobutton', undefined, '파일 이름의 섹션 prefix 사용');

			var startAtPage1 = g1.add ('checkbox {text: "각 문서의 첫 번째 Folio는 1"}');

			var folder_group = g1.add ('group');
				folder_group.add ('statictext {text: "출력 폴더:", characters: 10, justify: "right"}');
			...
			var panel = g1.add ('group {orientation: "row"}');
				panel.add ('statictext {text: "PDF 프리셋:", characters: 10, justify: "right"}');
			...
			var view_pdf = g1.add ('checkbox', undefined, '\u00A0출력 후 PDF 보기');
			
			var buttons = w.add ('group {orientation: "column", alignChildren: "fill"}');
				var select_all = buttons.add ('button', undefined, '모두 선택');
				var deselect_all = buttons.add ('button', undefined, '모두 선택 해제');
				var invert = buttons.add ('button', undefined, '선택 반전');
				var ok = buttons.add ('button', undefined, '확인', {name: 'ok'});
