...
var appName = "Object ALT Text Changer | by @tomaxxi | edit by @geonwoo | edit by @iM";

if(parseFloat(app.version) > 7){
    if(app.books.length < 1){
        alert("열려있는 북파일이 없습니다.", appName);
        exit();
    }else{
    	addAltText();
	}
...
function addAltText(){
	...
	for (var m=0; m< myDocs.length; m++) {
	 	app.open(File(myBook.bookContents[m].fullName.toString()));
...
if(counter > 0)
                alert("Complete: " + lang.selection.text, appName);
            else
                alert("No Images affected!", appName);
}
