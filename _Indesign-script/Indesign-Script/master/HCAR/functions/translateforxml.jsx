// translateforXML();
function translateforXML(myScriptFolderPath) {
	app.scriptPreferences.userInteractionLevel = UserInteractionLevels.NEVER_INTERACT;

	var myFolder = Folder.selectDialog( "현대기아자동차 매뉴얼 전용 번역 XML Export\r\r인디자인 문서가 있는 폴더를 선택하세요." );  
	if ( myFolder != null ) {
		var myFiles = [];
		GetSubFoldersX(myFolder);
		if ( myFiles.length > 0 ) {
	...
	function exportTRXML() {
		...
		if (indentXML.exists) {
			...
			alert('indent_XML.xsl 파일이 없습니다.');
		}
		
	}
	//=================================== FUNCTIONS =========================================
	function GetSubFoldersX(theFolder) {
		var myFileList = theFolder.getFiles();
		for (var i = 0; i < myFileList.length; i++) {
			var myFile = myFileList[i];
			if (myFile instanceof Folder){
				GetSubFoldersX(myFile);
			}
