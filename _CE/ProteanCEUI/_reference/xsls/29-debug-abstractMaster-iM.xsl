...
<xsl:stylesheet 
	...
	<xsl:variable name="debug" select="document(concat('file:/', replace($mPath, '\\', '/')))"/>
	<xsl:variable name="outdir" select="document(concat('file:/', replace($mPath, '\\', '/'), '/out'))"/>
	...
	<xsl:variable name="files" select="collection(concat('file:/', replace($mPath, '\\', '/'), '/out/2_HTML/debugs?select=*.html;recurse=yes'))"/>
	...
	<xsl:template match="/">
		<!--<xsl:variable name="filename" select="concat($debug, '/debugs-abstract.html')"/>-->
		<xsl:variable name="filename" select="concat($outdir, '/debugs-abstract.html')"/>
