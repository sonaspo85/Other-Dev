...
<xsl:stylesheet 
	...
	<xsl:variable name="Combined">
		<xsl:text>&#xA;</xsl:text>
		<root>
			<xsl:for-each select="$PV/root/listitem/item[@LV_name=$LV_name]">
	...
	<xsl:template match="item" mode="combine">
		...
		<xsl:element name="paragraph">
			...
			<xsl:if test="parent::listitem/@rev">
				<xsl:attribute name="rev" select="parent::listitem/@rev"/>
			</xsl:if>
	...
	<xsl:template name="getBase">
		...
		<xsl:choose>
			<xsl:when test="$this/@base and $this/@base = tokenize($Exist, '\|')[2]">
				<xsl:attribute name="base" select="$this/@base"/>
				<xsl:if test="$this/@rev">
					<xsl:attribute name="rev" select="$this/@rev"/>
				</xsl:if>
			</xsl:when>
			<xsl:when test="$this/@base and $this/@base != tokenize($Exist, '\|')[2]">
				<xsl:attribute name="base" select="tokenize($Exist, '\|')[2]"/>
				<xsl:attribute name="rev" select="$this/@base"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="base" select="tokenize($Exist, '\|')[2]"/>
			</xsl:otherwise>
