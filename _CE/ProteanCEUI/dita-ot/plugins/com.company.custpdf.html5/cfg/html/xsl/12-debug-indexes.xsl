...
<xsl:stylesheet 
	...
	<xsl:variable name="indexes" select="/CategoryData//*[@index='yes'][ancestor::page[not(@search='no')]]"/>
	...
	<xsl:template match="/">
		<xsl:variable name="filename" select="concat('../../2_HTML/debugs/', $LV_name, '/', 'indexes.html')" />
		<xsl:result-document href="{$filename}">
			<html>
				...
				<body onload="myFunction()">
					<xsl:value-of select="ast:indent(2)" />
    				<p id="summary" data-check="index" path="{ast:getPath(base-uri(.))}">
    					<span id="true">&#x200B;</span>
						<span id="false">&#x200B;</span>
    				</p>
    				<xsl:value-of select="ast:indent(2)" />
    				<table id="table">
    					...
    					<xsl:for-each select="$indexes">
    						...
    						<tr>
								...
								<xsl:variable name="id" select="if ( @id ) then @id else ancestor::*[@id][1]/@id"/>
				...
				<script>
					<xsl:value-of select="ast:indent(2)" />
					<xsl:text>document.getElementById("true").innerHTML = "index items = " + document.querySelectorAll('.true').length;</xsl:text>
					<xsl:value-of select="ast:indent(2)" />
