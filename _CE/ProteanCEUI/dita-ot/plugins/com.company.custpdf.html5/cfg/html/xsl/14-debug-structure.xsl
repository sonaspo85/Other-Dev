...
<xsl:stylesheet 
	...
	<xsl:template match="/">
		<xsl:variable name="filename" select="concat('../../2_HTML/debugs/', $LV_name, '/', 'structure.html')" />
		<xsl:result-document href="{$filename}">
			<html>
				...
				<body>
					<xsl:value-of select="ast:indent(2)" />
    				<p id="summary" data-check="structure" path="{ast:getPath(base-uri(.))}">
    					<span id="true">&#x200B;</span>
						<span id="false">&#x200B;</span>
    				</p>
    				<xsl:value-of select="ast:indent(2)" />
				...
				<script>
					<xsl:value-of select="ast:indent(2)" />
					<xsl:text>document.getElementById("true").innerHTML = "category items = " + document.querySelectorAll('.true').length;</xsl:text>
					<xsl:value-of select="ast:indent(2)" />
