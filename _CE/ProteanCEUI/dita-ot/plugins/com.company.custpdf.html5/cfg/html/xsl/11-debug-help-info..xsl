...
<xsl:stylesheet 
	...
	<xsl:variable name="h2s" select="/CategoryData//h2[@class='Heading2']"/>
	...
	<xsl:template match="/">
		<xsl:variable name="filename" select="concat('../../2_HTML/debugs/', $LV_name, '/', 'help-info.html')" />
		<xsl:result-document href="{$filename}">
			<html>
				...
				<body>
					<xsl:value-of select="ast:indent(2)" />
    				<p id="summary" data-check="info" path="{ast:getPath(base-uri(.))}">
    					<span id="true">&#x200B;</span>
						<span id="false">&#x200B;</span>
    				</p>
    				<xsl:value-of select="ast:indent(2)" />
    				<table id="table">
    					...
    					<xsl:for-each select="$h2s">
    						...
    						<tr>
    							...
    							<xsl:value-of select="ast:indent(4)" />
    							<td>
    								<xsl:value-of select="ancestor::chapter/@description"/>
    							</td>
    							<xsl:value-of select="ast:indent(4)" />
    							<td>
    							...
    							<xsl:value-of select="ast:indent(4)" />
    							<td>
    								<xsl:value-of select="ancestor::page/@description"/>
    							</td>
				...
				<script>
					<xsl:value-of select="ast:indent(2)" />
					<xsl:text>document.getElementById("true").innerHTML = "helo-info items = " + document.querySelectorAll('.true').length;</xsl:text>
					<xsl:value-of select="ast:indent(2)" />
