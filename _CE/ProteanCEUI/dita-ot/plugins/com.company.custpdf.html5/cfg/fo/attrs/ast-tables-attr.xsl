...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0">
	...
	<xsl:attribute-set name="common.table.body.entry">
		<xsl:attribute name="space-before">
		    <xsl:choose>
		    	<xsl:when test="@outputclass = 'no-margin'">0pt</xsl:when>
	...
	<xsl:attribute-set name="tbody.row.entry">
		...
		<xsl:attribute name="line-height">
			<xsl:choose>
		    	<xsl:when test="@outputclass = 'no-margin'">100%</xsl:when>
		    	<xsl:otherwise><xsl:value-of select="$default-line-height"/></xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
	</xsl:attribute-set>
