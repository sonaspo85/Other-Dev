...
<xsl:stylesheet exclude-result-prefixes="java" version="2.0" xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="image-style-rules">
		<!--
			...
			*.svg 확장자 추가
		-->
		<xsl:if test="not(contains('|jpg|jpeg|png|svg|',concat('|',substring-after(substring(@href,string-length(@href)-4), '.'),'|')))">
			<xsl:call-template name="failed-assert">
				<xsl:with-param name="rule-id">image-file-type-not-supported</xsl:with-param>
				<xsl:with-param name="test">not(contains('|jpg|jpeg|png|svg|',concat('|',substring-after(substring(@href,string-length(@href)-4), '.'),'|')))</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	<!--
